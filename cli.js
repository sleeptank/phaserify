...
const check_for_updates = spawn('npm', ['install','-D','phaser-ce','browserify'], {cwd: proj_path, stdio: 'inherit'})
...
check_for_updates.on('close', (code) => {
  if (code !== 0) {
    ...
    const bundle_phaser = spawn('browserify', [path.join(base_path, 'index.js'), '-o', bundle_path], {cwd: proj_path, stdio: 'inherit'})
